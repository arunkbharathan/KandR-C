...
float
two_raiseto_f (unsigned int exponent)
{
  unsigned int i = 1;
  float temp = 1;
  temp = pow (2, exponent);
...
double
two_raiseto_d (unsigned int exponent)
{
  unsigned int i = 1;
  double temp = 1;
...
long_double_max ()
{
  ...
  printf ("\nlong double max  2^(%u)", exponent);
  printf (" or \na little above 10^(4932)\n");
  printf ("and min is 2^-16382 (Wiki Search)\n\n");
}

long double
two_raiseto_ld (unsigned int exponent)
{
  unsigned int i = 1;
  long double temp = 1;
  while (i <= exponent)
...
fixed_range ()
{
  ...
  printf ("signed short int range %hi to %hi\n", ss_int, ssit);
  printf ("signed int range %i to %i\n", s_int, sit);
  printf ("signed long int range %li to %li\n", sl_int, slit);
  printf ("signed long long int range %lli to %lli\n", sll_int, sllit);
